...
```tsx
<SmartVideoPlayer path="Movies/Sintel.mp4" forceTranscode quality="high" />
```
...
export function MoviePlayer({ movie }: { movie: Movie }) {
  return (
    <Card className="p-4">
      ...
      <SmartVideoPlayer path={movie.path} poster={movie.poster} quality="high" className="w-full aspect-video" />
    </Card>
  );
}
```
...
export function VideoWithProgress({ path }: { path: string }) {
  ...
  return (
    <div>
      <SmartVideoPlayer path={path} onTimeUpdate={handleTimeUpdate} />
      <div className="mt-2 text-sm text-muted-foreground">Progress: {progress.toFixed(1)}%</div>
    </div>
  );
}
```
...
export function VideoWithOptions({ path }: { path: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <SmartVideoPlayer path={path} forceTranscode quality={quality} format={format} showStreamInfo />
    </div>
  );
}
```
...
export function AdaptiveVideoPlayer({ path }: { path: string }) {
  ...
  return <SmartVideoPlayer path={path} quality={quality} className="w-full" />;
}
```
