...
internal static class GlobalSettingsConfiguration
{
    ...
    internal static void UseGlobalSettings(this WebApplication app)
    {
        ...
        api.MapGet(
                "/",
                async (IGlobalSettingsApi settingsApi) =>
                {
        ...
        api.MapPut(
                "/",
                async (UpdateGlobalSettingsRequest request, IGlobalSettingsApi settingsApi) =>
                {
                    // Validate inputs
                    if (string.IsNullOrWhiteSpace(request.PreferredMetadataLanguage))
                    {
                        return Results.BadRequest(
                            new { error = "PreferredMetadataLanguage is required" }
                        );
                    }
                    ...
                    {
                        return Results.BadRequest(
                            new { error = "CountryCode must be a 2-letter ISO 3166-1 alpha-2 code" }
                        );
                    }
